...
export async function loadProjectById(id: string): Promise<(HydrateData & { id: string; name: string }) | null> {
  ...
  for (const meta of project.sources) {
    ...
    // Image thumbnails are downscaled JPEGs persisted separately (see
    // THUMBNAIL_STORE) so reloads never re-decode the full-res source; older
    // projects saved before that existed fall back to undefined here and get
    // lazily regenerated by getImageThumbnail() on first render.
    const thumbnail = kind === "image" ? await getPersistedThumbnail(meta.id) : meta.thumbnail;
...
/** Reads a persisted downscaled thumbnail (JPEG data URL) for a media source, if one exists. */
export async function getPersistedThumbnail(sourceId: string): Promise<string | undefined> {
  const db = await getDB();
  return (await db.get(THUMBNAIL_STORE, sourceId)) as string | undefined;
}

/** Persists a downscaled thumbnail (JPEG data URL) for a media source, keyed by source id. */
export async function putPersistedThumbnail(sourceId: string, dataUrl: string): Promise<void> {
  const db = await getDB();
  await db.put(THUMBNAIL_STORE, dataUrl, sourceId);
}
